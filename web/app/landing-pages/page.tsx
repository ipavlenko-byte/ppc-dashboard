...
export default async function LandingPagesPage({
  ...
  const { landingPages } = await getDashboardData();
  const campaigns = Array.from(new Set(landingPages.map((r) => r.campaign))).sort();

  const filtered = applyDateFilter(
    campaign ? landingPages.filter((r) => r.campaign === campaign) : landingPages,
    filter
  );
  const summaries = summarizeGeneric(filtered, (r) => r.landingPage);
  const total = grandTotalGeneric(summaries);

  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h1 className="text-xl font-semibold">Посадочные страницы — {filter.label}</h1>
      ...
      <MetricsTable rows={summaries} total={total} nameLabel="Страница" />
    </div>
  );
}
