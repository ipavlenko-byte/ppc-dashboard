...
11. И вкладку `gsc_totals_daily` — тотал по сайту без разбивки по query/page. Нужна потому, что Search Console по политике приватности скрывает часть редких запросов в детализированных отчётах (`gsc_query_daily`/`gsc_page_daily`), из-за чего их сумма всегда немного меньше настоящего тотала по сайту — цифры на KPI-плитках SEO → Обзор берутся именно из `gsc_totals_daily`, чтобы точно совпадать с "Total clicks/impressions" в самой Search Console.
