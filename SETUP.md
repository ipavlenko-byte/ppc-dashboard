...
9. Гео определяется по ISO-3166-1 alpha-3 коду страны (это трёхбуквенный код, не путать с двухбуквенными кодами в остальных частях дашборда) — в скрипте зашита таблица соответствий `GSC_COUNTRY_NAMES` для основных рынков. Если увидите `Unknown (код)` — допишите код в эту таблицу внутри скрипта.
